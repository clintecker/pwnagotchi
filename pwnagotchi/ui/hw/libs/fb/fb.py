...
def ready_fb(_bpp=None, i=0, layer=0, _win=None):
    ...
    with open("/dev/fb" + str(i), "r+b") as f:
        ...
        if _bpp:
            ...
            try:
                vi = ioctl(
                    f, FBIOPUT_VSCREENINFO, struct.pack("I" * 40, *vi)
                )  # fb_var_screeninfo
        ...
        w, h = (
            ll // bytepp,
            vi[1],
        )  # when screen is vertical, width becomes wrong. ll//3 is more accurate at such time.
...
def _888_to_565(bt):
    ...
    for i in range(0, len(bt), 3):
        b += int.to_bytes(
            bt[i] >> 3 << 11 | bt[i + 1] >> 2 << 5 | bt[i + 2] >> 3, 2, "little"
        )
    return b
...
def numpy_888_565(bt):
    ...
    return (
        (((0xF80000 & arr) >> 8) | ((0xFC00 & arr) >> 5) | ((0xF8 & arr) >> 3))
        .astype(np.uint16)
        .tostring()
    )
