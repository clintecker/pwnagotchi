...
class PawGPS(plugins.Plugin):
    ...
    def on_loaded(self):
        ...
        if (
            "ip" not in self.options
            or ("ip" in self.options and self.options["ip"] is None)
            or (len("ip" in self.options and self.options["ip"]) is 0)
        ):
            logging.info(
                "[paw-gps] no IP Address defined in the config file, will uses paw server default (192.168.44.1:8080)"
            )
    ...
    def on_handshake(self, agent, filename, access_point, client_station):
        ...
        try:
            ...
            try:
                ...
                logging.error(
                    f"[paw-gps] encountered error while saving gps data: {error}"
                )
        except Exception as error:
            logging.error(
                f"[paw-gps] encountered error while getting gps data: {error}"
            )
