...
class Agent(Client, Automata, AsyncAdvertiser, AsyncTrainer):
    def __init__(self, view, config, keypair):
        Client.__init__(
            ...
            (
                "127.0.0.1"
                if "hostname" not in config["bettercap"]
                else config["bettercap"]["hostname"]
            ),
            (
                "http"
                if "scheme" not in config["bettercap"]
                else config["bettercap"]["scheme"]
            ),
            8081 if "port" not in config["bettercap"] else config["bettercap"]["port"],
            (
                "pwnagotchi"
                if "username" not in config["bettercap"]
                else config["bettercap"]["username"]
            ),
            (
                "pwnagotchi"
                if "password" not in config["bettercap"]
                else config["bettercap"]["password"]
            ),
        )
        ...
        try:
            ...
            for ap in s["wifi"]["aps"]:
                if ap["encryption"] == "" or ap["encryption"] == "OPEN":
                    ...
                    ap["hostname"] in whitelist
                    or ap["mac"][:13].lower() in whitelist
                    or ap["mac"].lower() in whitelist
                ):
                    logging.info("whitelisted AP: %s (%s)", ap["hostname"], ap["mac"])
                    continue
                else:
                    aps.append(ap)
        except Exception as e:
    ...
    def _update_handshakes(self, new_shakes=0):
        ...
        if self._last_pwnd is not None:
            txt += (
                " [%s]" % self._last_pwnd[:11]
            )  # So it doesn't overlap with fix_brcmfmac_plugin
    ...
    def _fetch_stats(self):
        while True:
            ...
            try:
                ...
                logging.error(
                    "[agent:_fetch_stats] self.update_uptimes: %s" % repr(err)
                )
            ...
            try:
                ...
                logging.error(
                    "[agent:_fetch_stats] self.update_advertisements: %s" % repr(err)
                )
            ...
            try:
                ...
                logging.error(
                    "[agent:_fetch_stats] self.update_counters: %s" % repr(err)
                )
            try:
                self._update_handshakes(0)
            except Exception as err:
                logging.error(
                    "[agent:_fetch_stats] self.update_handshakes: %s" % repr(err)
                )
    ...
    async def _on_event(self, msg):
        ...
        try:
            plugins.on(
                "bcap_%s" % re.sub(r"[^a-z0-9_]+", "_", jmsg["tag"].lower()), self, jmsg
            )
        except Exception as err:
    ...
    def _event_poller(self, loop):
        ...
        while True:
            ...
            try:
                ...
                logging.debug(
                    "[agent:_event_poller] Error while polling via websocket (%s)", ex
                )
